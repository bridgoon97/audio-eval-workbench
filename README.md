...
- 草稿阶段“对齐与响度”：选参考候选做恒定整数采样延迟估计（±1600 samples，三窗口归一化互相关）与活动段 RMS 匹配（固定增益，非 LUFS）；先预览诊断再应用，生成独立派生试听资产，原始 WAV 不修改，可一键恢复。
- 草稿任务与片段信息可修正；进行中的任务可增补成员，未贡献记录的参与者可移出。
- 管理员、组织者与评测者账号；组织者可上传并管理自己的任务，任务删除进入可恢复的回收站。
- 自助申请加入：管理员生成一次性邀请凭证，同事在登录页填身份信息申请，管理员批准时设置角色与任务；批准后的浏览器获得长期设备凭证免密登录，管理员可随时查看和撤销设备。IP 只用于审计提示，绝不作为登录凭证。
- 账号生命周期：已登录同事可在“账号与安全”自助修改密码（密码会话核验旧密码，免密设备会话直接设置）；管理员可为丢失浏览器的同事签发一次性恢复凭证，本人领取时设置新密码；账号可停用/启用，停用立即撤销全部登录并保留历史记录。管理员账号不可停用，站点永远保留可用管理员。
...
- Agent 编排入口：完整服务端 Windows 包内置 `audio-eval.exe agent ...` 与 Agent 使用文档（`Agent使用入口.txt`、`docs/Agent创建评测任务.md`、清单 schema 与示例），同机 Agent 无需另装独立 CLI；文档含双入口命令映射（独立 Agent CLI 包为 `audio-eval-agent.exe ...`）。
...
1. 管理员在左侧“团队成员”创建账号，或在“加入与邀请”生成一次性邀请凭证，让同事在登录页自助申请、由管理员审批；需要创建/上传任务的同事选择“组织者”，只参与听评的同事选择“评测者”。
...
草稿阶段可安全纠正录入错误：编辑任务标题/类型/比较模式（已有标注或评分时比较模式冻结）、编辑片段名称，以及删除整个无贡献片段——删除会一并移除其候选、派生资产与不再被任何任务引用的音频，共享同一 SHA 的文件保留。已发布任务不可修订或删除片段。

管理员在回收站可对已软删除的任务执行“永久清除”：先预览将释放的片段/候选数量与可计算字节数，再凭服务端一次性确认令牌执行；只删除该任务独占的数据库记录与资产，被其他任务共享的文件绝不误删。清除后不可恢复，完整备份也不再包含该任务。

任务首页底部“回收站”可恢复自己有权管理的任务。恢复保留原有状态、成员、音频、标注和评分；恢复已发布任务不会解除输入冻结。管理员可对回收站任务永久清除并释放其独占空间（见上）；若个别文件暂时无法删除，会记录为待清理并在重试成功后释放。
...
有评测需求的同事不用等管理员手工建号：管理员在“加入与邀请”生成高熵一次性邀请凭证（可选团队邀请或绑定单个任务，带用途、有效期和最大使用次数），同事在登录页点击“申请加入评测”，填写显示名称、可选工号/邮箱和邀请凭证后提交。申请在批准前不能访问任何任务、音频或成员信息；重复提交同一浏览器只会得到同一条申请。

管理员在“加入与邀请”的申请列表中查看编号、身份字段、时间、来源 IP、简化设备信息与邀请用途，批准时设置账号名称（默认显示名称，可改）、角色（评测者/组织者）和正在评测中的任务，写入既有的成员与受邀名单；拒绝不消耗邀请次数，使用次数在批准时原子消耗，并发批准不会超额。凭证明文只在生成时显示一次，服务端只保存带独立盐的哈希；申请编号与领取秘密分离，领取秘密也只存哈希。

批准后，原浏览器通过一次性领取流程获得随机高熵设备令牌（`HttpOnly`、`SameSite=Lax` Cookie，180 天绝对过期），自动登录；刷新页面免密进入。数据库只保存令牌哈希；管理员可在“团队成员 → 登录设备”查看简化设备信息、首次/最近时间与 IP，并撤销单设备或全部设备，撤销立即生效。退出登录会同时撤销本浏览器设备。换浏览器或清空 Cookie 后，设备凭证不在原浏览器时，可由管理员重置密码或签发一次性恢复凭证；被批准账号初始没有可用密码。

**IP 不是登录凭证。** IP 只用于审计、风险提示和限流；同一设备令牌更换 IP 后仍可登录并更新审计，相同 IP 没有设备 Cookie 不能登录，程序不读取任何转发头（uvicorn 以 `proxy_headers=False` 启动），伪造 `X-Forwarded-For` 不能绕过限流或顶替身份。部署在反向代理后时，所有限流按代理地址共享口径，宁可偏严。申请提交、状态查询与领取均有限流；登录成功会重置该地址的失败尝试计数。

## 密码修改、恢复与账号停用

- **自助修改密码**：登录后点击左下角“账号与安全”。用账号密码登录的同事需输入当前密码；通过设备免密登录的同事可直接设置。保存后本浏览器保持登录，其他浏览器与设备立即退出。
- **一次性恢复凭证**：同事浏览器丢失且无法登录时，管理员在“团队成员 → 恢复凭证”生成短时效（1–72 小时）一次性凭证；本人在登录页“凭恢复凭证设置新密码”领取并重新登录。明文只在生成时显示一次，服务端只保存加盐哈希；错误、过期、已用或停用账号的领取一律拒绝，并发领取最多成功一次。
- **停用与启用**：停用立即撤销该同事所有会话与设备，密码登录、设备登录与恢复领取全部拒绝；评论、评分、任务归属与稳定用户 ID 原样保留。启用后需要重新登录，不恢复任何旧会话。管理员账号不能被停用。
...
- 对齐与响度只在草稿阶段由组织者手动应用：高置信建议才可勾选，低相关、多峰、漂移、活动不足、增益超 ±12 dB 或削波余量不足都会拒绝并给出原因；物理传感器或不同内容不一定能可靠相关。活动段 RMS 匹配不是 LUFS/ITU BS.1770 响度校准，也不是听感等响；发布后播放处理冻结。
...
- 批量导入按完整相对路径精确匹配，尚无手动拖拽映射与任意采样率转换；草稿任务/片段修订、整段删除与管理员永久清除已提供。
- 评测顺序中候选按人随机且稳定；尚无平衡题序、重复题协议、正式练习题或统计置信区间。
- 延迟/漂移检查与活动段响度匹配已随草稿工具提供；尚无算法 diagnostics、回归集和模型运行适配器。
- 账号生命周期（密码修改、账号停用、免密设备、一次性恢复凭证）已实现；尚无自动 HTTPS、后台服务安装、自动备份计划与免邮件的密码自助找回（无法登录时请管理员签发一次性恢复凭证）。
...
浏览器测试使用独立临时目录和端口 8877（可用 `E2E_PORT` 环境变量覆盖，避免同一主机上的多套验证互相占用），由 `uv run --locked` 启动。打包测试使用 8878。测试覆盖授权、匿名元数据、版本冻结、真实音频增益、篡改检测、10 人并发提交、备份恢复和真实 Web Audio 渲染；含 1 采样错位变异检查。
