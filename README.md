...
批准后，原浏览器通过一次性领取流程获得随机高熵设备令牌（`HttpOnly`、`SameSite=Lax` Cookie，180 天绝对过期），自动登录；刷新页面免密进入。数据库只保存令牌哈希；管理员可在“团队成员 → 登录设备”查看简化设备信息、首次/最近时间与 IP，并撤销单设备或全部设备，撤销立即生效。退出登录会同时撤销本浏览器设备。换浏览器或清空 Cookie 后，设备凭证不在原浏览器时需要管理员重置密码；被批准账号初始没有可用密码。
...
- 尚无密码自助修改/重置、账号停用、自动 HTTPS、后台服务安装和自动备份计划；被批准账号换浏览器登录需管理员重置密码。
