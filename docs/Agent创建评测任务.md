...
- state 同时绑定首次 `mapping.json` 的 SHA：后续任何 mapping 修改都会被拒绝（mapping 的输出必须能由源与声明的转换重建，自洽改写 SHA 指向其他 WAV 无法通过逐样本重建校验）。
...
4. 参与者全部在受邀名单。
5. 对齐/响度：服务端判据拒绝时——默认停止编排（拒绝证据持久化进 state，重跑与 `--publish` 都不能绕过）；仅当显式 `--keep-original-on-rejection` 才保留原始继续**草稿**，且该开关下永不发布。已处理轨按服务端回执的派生 SHA 复核，原始 SHA 证据保留在 mapping。
6. `manifest.publish=true` **且** `--publish` 同时满足才调用发布；发布后任务冻结，不可继续修改。
7. **owner 不隐式受邀**：participants 为空时发布的任务没有受邀评测者（owner 保留管理权但不承担评分义务）；参与者解析会拒绝不存在的、重名的与已停用的账号。
